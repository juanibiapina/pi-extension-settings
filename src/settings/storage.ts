...
import { ExtensionAPI, getAgentDir } from "@mariozechner/pi-coding-agent";
import type {
 ...
 */
export function getSettingsForExtension(
...
function createSettingsDefinitionsFromSchema(
  schema: StandardSchemaV1<any, any>,
): SettingDefinition[] {
...
export interface TypedSettingsStore<Output> {
   ...
   * @returns The setting value
   */
  get<K extends keyof Output>(
    key: K,
    fallback?: Output[K],
  ): Promise<Output[K] | undefined>;
...
>(
  ...
  pi: ExtensionAPI,
) => TypedSettingsStore<StandardSchemaV1.InferOutput<Schema>> {
  ...
  ): Promise<{ value: Output } | { issues: unknown }> {
    return (await schema["~standard"].validate(raw as Input)) as
      | { value: Output }
      | { issues: unknown };
  }
  ...
  return (pi) => {
    ...
    return {
      async get<K extends keyof Output>(
        key: K,
        fallback?: Output[K],
      ): Promise<Output[K] | undefined> {
      ...
      async getAll(fallbacks?: Partial<Output>): Promise<Output> {
        ...
        return (
          fallbacks ? { ...fallbacks, ...result.value } : result.value
        ) as Output;
      },
    };
  };
